...
const LEGACY_PLAN_NAMES = ['Mien phi', 'Tieu chuan', 'Nang cao', 'Doanh nghiep']
...
async function main () {
  ...
  const demoUsers = [
    { email: 'admin@demo.school.vn', fullName: 'Quản trị viên', role: 'SUPER_ADMIN', password: hashedPasswords.admin },
    { email: 'staff@demo.school.vn', fullName: 'Nhân viên Giáo vụ', role: 'STAFF', department: 'Giáo vụ', password: hashedPasswords.staff },
    { email: 'teacher@demo.school.vn', fullName: 'Nguyễn Văn Thầy', role: 'TEACHER', department: 'Toán', password: hashedPasswords.teacher }
  ]
  ...
  for (const student of activeYearStudents.slice(3, 24)) {
    ...
    const generatedParent = await prisma.user.upsert({
      ...
      update: {
        password: hashedPasswords.parent,
        fullName: `Phụ huynh ${student.fullName}`,
