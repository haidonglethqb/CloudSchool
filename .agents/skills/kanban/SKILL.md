...
**IMPORTANT:** Run server as Codex background task using `run_in_background: true` with the Bash tool. This makes the server visible in `/tasks` and manageable via `KillShell`.

The skill is located at `.Codex/skills/plans-kanban/`.
...
```bash
node .Codex/skills/plans-kanban/scripts/server.cjs --stop
```
...
node .Codex/skills/plans-kanban/scripts/server.cjs \
  --dir "$PLANS_DIR" \
  --host 0.0.0.0 \
  --open \
  --foreground
...
```json
{
  "command": "node .Codex/skills/plans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
