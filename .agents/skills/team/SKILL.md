...
**Requires:** Agent Teams enabled. Set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json env if using Codex < 2.1.33. May be GA in 2.1.33+.
...
Agents with `memory: project` retain learnings across team sessions. Memory persists in `.Codex/agent-memory/<name>/` (gitignored). Useful for:
...
See `.Codex/rules/team-coordination-rules.md` for teammate behavior rules.
